...
Every real bug found so far fell into one of **forty-two shapes**. Probe for these after any change. The list earns its keep: the last three sessions each found a defect by walking it rather than by a test failing, and the count is checked against the table rather than remembered — it read "twenty-seven" for a fortnight while the table held forty.

---

## The forty-two shapes
