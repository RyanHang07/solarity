...
Every real bug found so far fell into one of **forty-nine shapes**. Probe for these after any change. The list earns its keep: the last three sessions each found a defect by walking it rather than by a test failing, and the count is checked against the table rather than remembered — it read "twenty-seven" for a fortnight while the table held forty.

---

## The forty-nine shapes
...
| **One function, two callers, one of them not what the change was for** | `complete_onboarding` has been both the signup path and the Settings rename path since step 3. Adding terms acceptance to it recorded consent when somebody renamed their username — a real timestamp, in the column whose purpose is proving consent, for a moment nobody was shown anything. The `coalesce` guarding it was solving the adjacent half: it stopped a rename *moving* an acceptance and let one *create* one. **Before adding a write to a shared function, list its callers and ask what the new line means for each.** The function already knew which path it was on |
| **An API that has not noticed the database changed** | dropping `complete_onboarding(text, text)` and creating `(text, text, text)` is correct SQL and left PostgREST serving a cached schema, so every three-argument call returned `PGRST202` and the app said "Something went wrong. Please try again." Nothing was wrong and retrying could not help. `notify pgrst, 'reload schema';` is the fix; **`toMessage` now has a branch for it, because the only person who ever sees this is whoever just deployed.** `create or replace` on an identical signature is unaffected: it is specifically drops, renames and signature changes |
| **A branch whose condition was never once true** | `sw.js` has routed digest taps to `?tab=overview` since step 10, gated on `d.type === "digest"`. The sender builds `data` as `{ notification_id, ...payload }`, and **no payload has ever contained a `type` key** — checked against all 138 rows in the table. The branch has never run, and nothing failed: taps went to the Circle's default tab, which is a reasonable place to land, so there was no symptom to notice. **A conditional reading a field nobody writes is the "declared with no writer" shape wearing a different hat**, and the way to find it is to ask what actually populates the value rather than reading the branch |
| **A destination that moved in one surface and not the other** | step 18 pointed the in-app `invited` notification at `/join/<token>`, because the recipient is not a member yet and `/circles/<id>` would bounce them. `sw.js` routes *push* taps by `group_id` and was never updated, so tapping the notification on a phone landed on a Circle they cannot see. **One event, two renderers, and only one of them was in the diff.** Same session also moved `start_url` to `/dashboard` for the landing page and left the worker's fallback at `/`, which would have opened the marketing page from inside the installed app. When a route changes, grep for the path *and* for what computes it |
| **A function in `public` is an API endpoint, whatever it was written for** | migration 103 created three trigger functions, and PostgREST exposed all three at `/rest/v1/rpc/` to `anon` and `authenticated`. Migration 7 wrote this rule down in August for `handle_new_user`; I did not follow it, and the platform's own linter found it rather than any reading of the diff. Not exploitable — plpgsql refuses a trigger function called outside a trigger — and that is not the defence anybody chose. **The tell: any `create function` in `public` needs a matching `revoke execute`, or a reason it is public.** Triggers execute as the table owner and never consult the grant, so revoking costs nothing. Run `get_advisors` after any migration that adds a function |
| **A definer function that resolves an id and never filters by it** | `circle_preview_members` took an invite token, checked the link was live, refused when it was not, and then selected from `group_members` with **no `where group_id = v_group_id`**. It returned the first ten rows across every Circle in the database to anybody holding any live link. Three things hid it: the guard clause above is the part that *looks* like the security and it was correct; `limit 10` is the size a right answer would also be; and `SECURITY DEFINER` means there is no RLS underneath to catch what the query forgets. The inverse of "RLS is not a substitute for a WHERE clause" — here there was no RLS at all. **In a definer function, read the query, not the guard**, and check that every identifier it resolves is used by the statement that returns rows. Caught by reading it back before writing the migration file; fixed by 100 |
| **A default that answers a question the read never answered** | `pushSubscribed` ignored `error` and returned `(count ?? 0) > 0`. A refused or failed read gives `count: null`, so the coalesce turned "I do not know" into "definitely not", and settings drew a **Turn on notifications** button for a device whose row was in the table. `pushEnabledHere` had the same shape one level up, where a service worker that never became ready was indistinguishable from no subscription. **A `??` on the result of a read is a claim that absence and failure mean the same thing**, and for anything a person will act on, they do not. The fix is a third state the UI renders honestly, not a better default. Cousin of "RLS filters silently": there the read returns fewer rows, here it returns none and a null count |
...
| **A new gate is a new precondition for every signed-in test** | the terms interstitial shipped correct and broke ten tests across four files — admin, avatar, circle-activity and the whole of dashboard — each reporting a missing landmark on a page that was actually `/onboarding/terms`. The three fixture accounts predate the migration, so the gate caught all of them. **The failures name the assertion, never the redirect**, which is why ten unrelated-looking reports had one cause; `/today` did exactly this in step 9 and `auth.setup.ts` has carried a two-paragraph comment about it since. **Adding a check to `app/(app)/layout.tsx` means adding a line to the setup project in the same commit** |
...
| **A decoy that satisfies the wait the real element would have failed** | `textContent()` *does* wait — for its element to be **attached**, which is a weaker promise than it sounds. Next's dev overlay keeps an empty `alert` node on every page, so `getByRole("alert").textContent()` resolved against it instantly, returned `""`, and failed with "no error shown for a wrong password" over a screenshot with the error plainly on screen. **The decoy also hid the ambiguity**: two matching nodes would have been a strict-mode violation, but the empty one matched *alone* precisely when the real one had not rendered — so the query was least safe exactly when it was least suspicious. Two defences, and the second needs no knowledge of the overlay: name the element the app renders (`p[role="alert"]`), and require it to say something (`.filter({ hasText: /\S/ })`). `push.spec.ts` had already hit this and fixed it inline; a third copy is what moved it into `e2e/ui.ts` |
| **A label that names more than the label** | `getByLabel("Password", { exact: true })` matched nothing and seven tests waited out the full timeout, each reporting a form that would not enable. A `<label>` contributes **all** its text to the accessible name, and this one wrapped the input, the rule hint and the error, so the field was really called "Password At least 8 characters, with a letter and a number." — longer still once it was invalid. Wrapping everything in one `<label>` is the shortest way to lay a field out and it silently renames the field. **The accessibility defect came first**: a screen reader announced the rule as the name and again as the description, and an invalid field got renamed by being wrong. So the fix was the component, not the locator — label holds the label, `aria-describedby` holds the rest. The tell is a timeout on a field you can see in the snapshot; the snapshot prints the computed name |
