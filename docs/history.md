...
| "They couldn't pick their sun colour" | **There was no picker, and the first answer was to show rather than ask.** Sun colour is derived from the user id by `memberSun.ts` — six presets, hashed, stable across devices, no schema and no settings screen — which was the deliberate answer to goal cosmetics being cut. So onboarding's first-goal screen began **drawing** the sun beside the form: not a preview, the sun they already have and every Circle will show them. **Then the picker was built anyway** — migration 111 — because showing somebody a colour they did not choose answers the wrong half of the question. It is onboarding-only on purpose: chosen once, beside the canvas that shows it, an identity rather than a setting. `null` still means "derive from my id", which keeps a failed write cosmetic rather than a wall in front of sign-up |
