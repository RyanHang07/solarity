...
- **Renderer: PixiJS.** WebGL-backed sprite and particle performance on iOS Safari. Budget ramp-up time for the scene graph, sprite batching and texture atlases as its own task, rather than learning it mid-build.
...
- **Cross-user visibility**: opt-in per user, viewable through a lazy-loaded tab on that person's profile rather than inline in the composite view.
...
- **Per-user snapshot** in Redis keyed by `user_id`, rewritten on any galaxy-affecting write, with a live Postgres fallback on miss.
- **Per-group composite** in a separate Redis hash (`group_composite:{group_id}`), patched per member. A single `HGETALL` bounded at 10 entries, rather than recomputing from full snapshots.
